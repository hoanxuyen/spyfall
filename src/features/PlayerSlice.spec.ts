...
describe("PlayerSlice's Reducer", () => {
  const initialState = {
    ...
    locations: [
      ...
      },
      {
        ...
        image: "/bao-tang-chien-tranh.jpeg",
      },
      {
      ...
      },
      {
        ...
        image: "/cong-vien-le-thi-rieng.jpg",
      },
    ],
    customLocations: [],
  ...
  describe("assignLocation reducer", () => {
    it("Should assign a random location from predefined list", () => {
      ...
      expect(newState.location).toBe(initialState.locations[0].name);
    });
  ...
  describe("showOption reducer", () => {
    it("Should set the locations option", () => {
      const newState = playerReducer(
        initialState,
        showOption(LocationSource.CUSTOM)
      );
